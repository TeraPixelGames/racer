...
const PLAN_CONTRACT := {
	...
	"vertical_links": [
		...
		{"id": "AtticRearStairUpperHallToAttic", "type": "stacked_stairwall_attic_stair", "from_floor": "upper", "to_floor": "attic", "lower_zone": "upper_front_hall", "upper_zone": "attic_toy_course", "lower_landing_center": Vector3(ATTIC_STAIR_X, 52.60, ATTIC_STAIR_LOWER_Z), "upper_landing_center": Vector3(ATTIC_STAIR_X, 104.60, ATTIC_STAIR_UPPER_Z), "stairwell_bounds": {"min": ATTIC_STAIR_BOUNDS_MIN, "max": ATTIC_STAIR_BOUNDS_MAX}, "opening_required": "attic_stair_ceiling_opening", "path_segments": ["lower_landing", "stair_flight", "attic_hatch_landing"], "continuity_gate": "visible treads, risers, stringers, rails, and landings must span upper floor datum to attic floor datum along the same front-to-back stairwall band as the main stair and overlap the hatch opening", "source_asset": "generated_floor_plan_architect_measured_stacked_attic_stair_v1", "collision_policy": "visible_architectural_attic_stair_collision", "validation_gate": "must not intersect attic route corridor, roof planes, upper living walls, or third-person camera views"},
	],
}
...
const TOYBOX_TREE_SWING_PATH := "res://assets/gameplay/tracks/home_yard_v3/props/toybox_tree_swing/stylized_realistic_tree_tire_swing.glb"
...
func _add_upper_hall_landing_floor(root: Node3D, parent: Node3D) -> void:
	...
	holder.set_meta("floor_coverage_contract", "visible finish floor covers the upper hall west of the straight stair opening and the north landing approach; x >= 48 and z 18..146 remains the intentional stairwell void")
...
func _add_attic_interior(root: Node3D, parent: Node3D) -> void:
	...
	finishes.set_meta("shell_footprint_contract", "attic deck/edge storage covers the main gambrel shell footprint x -200..90 z -130..145, with route storage inset inside the walkable roof volume")
	...
	_add_room_floor(root, finishes, "AtticFloorDeckEastEaveShellStrip", Vector3(77.5, 103, 7.5), Vector3(25, 2, 275), Color(0.40, 0.29, 0.19), false)
...
func _add_scheduled_wall(root: Node3D, parent: Node3D, wall: Dictionary, color: Color) -> void:
	...
	var openings := _scheduled_wall_openings(wall)
	if openings.size() > 0:
		var sorted_openings := openings.duplicate()
		sorted_openings.sort_custom(func(a: Dictionary, b: Dictionary) -> bool:
			var a_span := a["span"] as Vector2
			var b_span := b["span"] as Vector2
			return minf(a_span.x, a_span.y) < minf(b_span.x, b_span.y)
		)
		var cursor := start
		for i in range(sorted_openings.size()):
			var opening_data := sorted_openings[i] as Dictionary
			var opening := opening_data["span"] as Vector2
			var a := clampf(minf(opening.x, opening.y), start, end)
			var b := clampf(maxf(opening.x, opening.y), start, end)
			var suffix := str(opening_data.get("id", "Opening%02d" % i))
			_add_scheduled_wall_segment(root, parent, wall, "%sSegment%02d" % [wall_id, i], cursor, a, color)
			_add_scheduled_opening_trim(root, parent, wall, "%s%s" % [wall_id, suffix], a, b, color)
			cursor = b
		_add_scheduled_wall_segment(root, parent, wall, "%sSegment%02d" % [wall_id, sorted_openings.size()], cursor, end, color)
	else:
		_add_scheduled_wall_segment(root, parent, wall, wall_id, start, end, color)

func _scheduled_wall_openings(wall: Dictionary) -> Array:
	if wall.has("opening_spans"):
		return wall["opening_spans"] as Array
	var opening := wall.get("opening_span", Vector2.ZERO) as Vector2
	if opening == Vector2.ZERO:
		return []
	return [{"id": "Opening", "span": opening}]

func _add_scheduled_opening_trim(root: Node3D, parent: Node3D, wall: Dictionary, node_prefix: String, a: float, b: float, color: Color) -> void:
	var axis := str(wall["axis"])
	var base_y := float(wall["base_y"])
	var height := float(wall["height"])
	if axis == "x":
...
func _add_opening_assemblies(root: Node3D, parent: Node3D) -> void:
	...
	var trim := Color(0.88, 0.82, 0.67)
	_add_box(root, parent, "FrontDoorPanel", Vector3(-50, 17, 146.6), Vector3(24, 34, 2.4), Color(0.24, 0.13, 0.07), false)
	...
	_add_box(root, parent, "KitchenPatioDoorGlass", Vector3(-128, 22, -135), Vector3(50, 30, 1.2), Color(0.48, 0.72, 0.86, 0.45), false, 0.0, Vector3.ZERO, _rear_facade_provenance("KitchenPatioDoorGlass", "kitchen patio glass is inset behind the patio frame opening and backed by a real rear opening"))
	...
	_add_box(root, parent, "PlayroomPatioDoorGlass", Vector3(20, 22, -135), Vector3(36, 30, 1.2), Color(0.48, 0.72, 0.86, 0.45), false, 0.0, Vector3.ZERO, _rear_facade_provenance("PlayroomPatioDoorGlass", "playroom patio glass is inset behind the patio frame opening and backed by a real rear opening"))
...
func _add_vertical_connectors(root: Node3D, parent: Node3D) -> void:
	...
	_add_scene(root, parent, KENNEY_STEPS_PATH, Vector3(ATTIC_STAIR_X, 52.70, ATTIC_STAIR_LOWER_Z), 0.0, Vector3(14.0, 14.0, 14.0), "AtticRearStairKenneyStepsReference")
	...
	_hide_visual_reference_stair(parent.get_node_or_null("AtticRearStairKenneyStepsReference"))
	_add_vertical_link_marker(root, parent, "AtticStairCeilingOpening", "AtticRearStairUpperHallToAttic", Vector3(72, 104.65, -46), Vector3(24, 2, 108))
...
func _vertical_circulation_contract() -> Dictionary:
	return {
		...
		"attic_stair": {
			"id": "AtticRearStairUpperHallToAttic",
			"type": "stacked_stairwall_attic_stair",
			...
			"width_units": 16.0,
			"tread_depth_units": 80.0 / 14.0,
			"riser_height_units": (ATTIC_ROOM_FLOOR_TOP_Y - UPPER_ROOM_FLOOR_TOP_Y - 0.60) / 14.0,
			"path_segments": [
				{"id": "lower_landing", "node": "AtticRearStairLowerLandingSurface", "center": Vector3(ATTIC_STAIR_X, UPPER_ROOM_FLOOR_TOP_Y - 0.30, ATTIC_STAIR_LOWER_Z), "size": Vector3(18, 0.6, 22)},
				{"id": "stair_flight", "node_prefix": "AtticRearStairTread", "start": Vector3(ATTIC_STAIR_X, UPPER_ROOM_FLOOR_TOP_Y + 0.60, ATTIC_STAIR_LOWER_Z), "end": Vector3(ATTIC_STAIR_X, ATTIC_ROOM_FLOOR_TOP_Y, ATTIC_STAIR_UPPER_Z), "tread_count": 14},
				{"id": "attic_hatch_landing", "node": "AtticRearStairUpperLandingSurface", "center": Vector3(ATTIC_STAIR_X, ATTIC_ROOM_FLOOR_TOP_Y - 0.30, ATTIC_STAIR_UPPER_Z), "size": Vector3(18, 0.6, 22)},
			],
			"opening_node": "AtticStairCeilingOpening",
			"opening_overlap_required": true,
			"continuous_path_verified": true,
			"validation_cameras": ["ValidationCameras/AtticRearStairContinuityCamera"],
		},
	}
...
func _hide_visual_reference_stair(node: Node) -> void:
	if node == null:
		return
	node.set_meta("visual_reference_hidden_by_measured_stair", true)
	if node is Node3D:
		(node as Node3D).visible = false

func _add_stair_stringer(root: Node3D, parent: Node3D, node_name: String, position: Vector3, size: Vector3, color: Color, yaw_degrees: float) -> void:
	var stringer := _add_box(root, parent, node_name, position, size, color, false, yaw_degrees, Vector3(-25, 0, 0))
	_tag_authored_vertical_asset(stringer, "stair_stringer")

func _add_stair_stringer_pair(root: Node3D, parent: Node3D, prefix: String, start: Vector3, end: Vector3, width: float, color: Color) -> void:
	...
	var pitch := rad_to_deg(atan2(end.y - start.y, run_length))
	var center := start.lerp(end, 0.5)
	center.y += 1.0
	var side_axis := Vector3(direction.z, 0.0, -direction.x).normalized()
	for side in [
		{"name": "Left", "offset": -width * 0.5 - 0.8},
		{"name": "Right", "offset": width * 0.5 + 0.8},
	]:
		var side_center := center + side_axis * float(side["offset"])
		var stringer := _add_box(root, parent, "%s%s" % [prefix, str(side["name"])], side_center, Vector3(1.5, 3.0, run_length + 2.0), color, false, yaw, Vector3(-pitch, 0, 0))
		_tag_authored_vertical_asset(stringer, "stair_stringer")
		stringer.set_meta("stair_flight_side", side["name"])
		stringer.set_meta("stair_flight_centerline_start", start)
		stringer.set_meta("stair_flight_centerline_end", end)

func _add_stair_guardrail(root: Node3D, parent: Node3D, node_name: String, position: Vector3, size: Vector3, color: Color) -> void:
	var rail := _add_box(root, parent, node_name, position, size, color, false)
	_tag_authored_vertical_asset(rail, "stair_guardrail")
...
func _add_stair_flight(root: Node3D, parent: Node3D, prefix: String, link_id: String, start: Vector3, end: Vector3, width: float, tread_count: int, color: Color) -> void:
	...
	var riser_prefix := prefix.replace("Tread", "Riser")
	for i in range(tread_count):
		...
		_tag_authored_vertical_asset(tread, "measured_tread")
		...
		var riser_center := start.lerp(end, float(i + 1) / float(tread_count))
		riser_center.y = top_y - rise * 0.5
		var riser := _add_box(root, parent, "%s%02d" % [riser_prefix, i], riser_center, Vector3(width, maxf(rise - 0.2, 0.5), 0.55), color.darkened(0.08), false, yaw)
		_tag_vertical_link(riser, link_id, "measured_riser", "main", "upper")
		_tag_authored_vertical_asset(riser, "measured_riser")
		riser.set_meta("vertical_path_continuity", "connects_main_floor_to_upper_floor_and_fills_between_treads")
		riser.set_meta("riser_index", i)
		riser.set_meta("riser_height_units", rise)
		riser.set_meta("target_dimensions_units", Vector3(width, maxf(rise - 0.2, 0.5), 0.55))

func _tag_authored_vertical_asset(node: Node, role: String) -> void:
	if node == null:
		return
	node.set_meta("temporary_stand_in", false)
	node.set_meta("validation_only_visible_placeholder", false)
	node.set_meta("replacement_source", "generated_floor_plan_architect_measured_stair_v2")
	node.set_meta("asset_role", role)
	node.set_meta("asset_lifecycle_state", "authored_measured_stair")
	node.set_meta("scale_contract_id", SCALE_CONTRACT_ID)
	node.set_meta("scale_validation_status", "floor_plan_architect_measured_visible_stair")
	node.set_meta("collision_policy", "visible_architectural_stair_collision")

func _add_attic_stair_continuity_geometry(root: Node3D, parent: Node3D) -> void:
	var wood := Color(0.52, 0.36, 0.22)
	var start := Vector3(ATTIC_STAIR_X, UPPER_ROOM_FLOOR_TOP_Y + 0.60, ATTIC_STAIR_LOWER_Z)
	var end := Vector3(ATTIC_STAIR_X, ATTIC_ROOM_FLOOR_TOP_Y, ATTIC_STAIR_UPPER_Z)
	_add_attic_stair_landing(root, parent, "AtticRearStairLowerLandingSurface", "lower_landing", Vector3(ATTIC_STAIR_X, UPPER_ROOM_FLOOR_TOP_Y - 0.30, ATTIC_STAIR_LOWER_Z), Vector3(18, 0.6, 22), wood.darkened(0.04))
...
func _add_attic_stair_landing(root: Node3D, parent: Node3D, node_name: String, part: String, position: Vector3, size: Vector3, color: Color) -> void:
	var landing := _add_box(root, parent, node_name, position, size, color, false)
	_tag_vertical_link(landing, "AtticRearStairUpperHallToAttic", part, "upper", "attic")
	_tag_authored_attic_stair_asset(landing, part)
	landing.set_meta("vertical_path_continuity", "connects_upper_floor_to_attic_floor")
	landing.set_meta("support_surface_top_y", position.y + size.y * 0.5)
	landing.set_meta("target_dimensions_units", size)

func _add_attic_stair_flight(root: Node3D, parent: Node3D, prefix: String, start: Vector3, end: Vector3, width: float, tread_count: int, color: Color) -> void:
	var horizontal := Vector3(end.x - start.x, 0.0, end.z - start.z)
	var run_length := horizontal.length()
	if run_length <= 0.01 or tread_count <= 0:
		return
	...
	for i in range(tread_count):
		...
		var center := start.lerp(end, t)
		center.y = top_y - 0.35
		var tread := _add_box(root, parent, "%s%02d" % [prefix, i], center, Vector3(width, 0.7, tread_depth + 0.5), color.lightened(float(i % 3) * 0.025), false, yaw)
		_tag_vertical_link(tread, "AtticRearStairUpperHallToAttic", "measured_tread", "upper", "attic")
		_tag_authored_attic_stair_asset(tread, "measured_tread")
		tread.set_meta("vertical_path_continuity", "connects_upper_floor_to_attic_floor")
		tread.set_meta("tread_index", i)
		tread.set_meta("tread_count", tread_count)
		tread.set_meta("riser_height_units", rise)
		tread.set_meta("tread_depth_units", tread_depth)
		tread.set_meta("target_dimensions_units", Vector3(width, 0.7, tread_depth + 0.5))
		var riser_center := start.lerp(end, float(i + 1) / float(tread_count))
		riser_center.y = top_y - rise * 0.5
		var riser := _add_box(root, parent, "AtticRearStairRiser%02d" % i, riser_center, Vector3(width, maxf(rise - 0.2, 0.5), 0.55), color.darkened(0.08), false, yaw)
		_tag_vertical_link(riser, "AtticRearStairUpperHallToAttic", "measured_riser", "upper", "attic")
		_tag_authored_attic_stair_asset(riser, "measured_riser")
		riser.set_meta("vertical_path_continuity", "connects_upper_floor_to_attic_floor_and_fills_between_treads")
		riser.set_meta("riser_index", i)
		riser.set_meta("riser_height_units", rise)
		riser.set_meta("target_dimensions_units", Vector3(width, maxf(rise - 0.2, 0.5), 0.55))

func _add_attic_stair_stringer_pair(root: Node3D, parent: Node3D, prefix: String, start: Vector3, end: Vector3, width: float, color: Color) -> void:
	var horizontal := Vector3(end.x - start.x, 0.0, end.z - start.z)
	var run_length := horizontal.length()
	if run_length <= 0.01:
		return
	...
	for side in [
		...
		var stringer := _add_box(root, parent, "%s%s" % [prefix, str(side["name"])], center + side_axis * float(side["offset"]), Vector3(1.5, 3.0, run_length + 2.0), color, false, yaw, Vector3(-pitch, 0, 0))
		_tag_vertical_link(stringer, "AtticRearStairUpperHallToAttic", "stair_stringer", "upper", "attic")
		_tag_authored_attic_stair_asset(stringer, "stair_stringer")

func _add_attic_stair_guardrail(root: Node3D, parent: Node3D, node_name: String, position: Vector3, size: Vector3, color: Color) -> void:
	var rail := _add_box(root, parent, node_name, position, size, color, false)
	_tag_vertical_link(rail, "AtticRearStairUpperHallToAttic", "stair_guardrail", "upper", "attic")
	_tag_authored_attic_stair_asset(rail, "stair_guardrail")

func _tag_authored_attic_stair_asset(node: Node, role: String) -> void:
	if node == null:
		return
	node.set_meta("temporary_stand_in", false)
	node.set_meta("validation_only_visible_placeholder", false)
	node.set_meta("replacement_source", "generated_floor_plan_architect_measured_stacked_attic_stair_v1")
	node.set_meta("asset_role", role)
	node.set_meta("asset_lifecycle_state", "authored_measured_attic_stair")
	node.set_meta("scale_contract_id", SCALE_CONTRACT_ID)
	node.set_meta("scale_validation_status", "floor_plan_architect_measured_visible_attic_stair")
	node.set_meta("collision_policy", "visible_architectural_attic_stair_collision")
...
func _add_validation_cameras(root: Node3D, parent: Node3D) -> void:
	...
	_add_camera(root, parent, "AtticRampSideProfileCamera", Vector3(20, 124, -46), Vector3(-8, -90, 0), 70)
	_add_camera(root, parent, "AtticRearStairContinuityCamera", Vector3(30, 118, 14), Vector3(-24, -142, 0), 54)
