...
func test_level_select_uses_optimized_backyard_preview_dressing() -> void:
	...
	assert_true(bool(screen.call("preview_has_visible_grid_road_for_test")), "Home-yard backyard preview should keep the active GridMap track visible")
...
func test_level_select_single_flow_actions_prepare_local_and_multiplayer() -> void:
	...
	assert_true(labels.has("Free Roam"), "Single flow should show home free-roam action")
	assert_true(labels.has("Race This Stage"), "Single flow should show local race action")
	assert_true(labels.has("Tournament"), "Single flow should show local tournament action")
	assert_true(labels.has("Multiplayer Lobby"), "Single flow should show multiplayer lobby action")
	assert_equal(screen.call("prepare_free_roam_flow_for_test"), "res://scenes/Race.tscn", "Free roam should route through the shared race runtime")
	assert_equal(NakamaService.get_meta_value("race_mode", ""), "home_free_roam", "Free roam should write home free-roam mode")
	assert_equal(NakamaService.get_meta_value("track_map_id", ""), "home_yard_v3", "Free roam should target the home-yard map")
...
func test_level_select_tournament_flow_actions_prepare_local_and_multiplayer() -> void:
	...
	assert_true(bool(screen.call("select_track_for_test", "kitchen")), "Level select should expose selectable kitchen track")
	var labels: Array = screen.call("get_flow_action_labels_for_test")
	assert_true(labels.has("Free Roam"), "Tournament flow should still expose home free roam")
	assert_true(labels.has("Race This Stage"), "Tournament flow should still expose one-off race action")
	...
	assert_equal(screen.call("prepare_tournament_flow_for_test"), "res://scenes/Race.tscn", "Tournament local should route directly to race")
...
func test_race_pause_menu_can_resume_or_target_level_select() -> void:
	...
	assert_true(labels.has("Return to Free Roam"), "Race pause should expose return to free roam")
...
func test_home_free_roam_spawns_player_and_ai_on_home_navigation_graph() -> void:
	...
	assert_true(labels.has("Start Race"), "Free roam pause should expose race start")
	assert_true(labels.has("Main Menu"), "Free roam pause should expose main menu")
	race.call("resume_race_for_test")
	race.queue_free()

func test_home_builder_is_disabled_during_local_races() -> void:
	var race: Node = _make_local_race()
	assert_true(not bool(race.call("live_builder_exists_for_test")), "Local races on home maps should not mount the live builder")
	race.queue_free()
