...
func test_old_home_yard_map_is_hidden_from_catalog() -> void:
	...
	assert_true(not ResourceLoader.exists("res://assets/gameplay/tracks/home_yard/home_yard_map.tscn"), "Old home_yard generated map should be deleted from runtime assets")
	assert_true(not ResourceLoader.exists("res://assets/gameplay/tracks/home_yard_v2/home_yard_v2_map.tscn"), "Old home_yard_v2 generated map should be deleted from runtime assets")

func test_retired_home_attempt_maps_are_removed_from_catalog() -> void:
	for map_id in ["home_estate_v1", "home_plan50_622_v1"]:
		assert_true(TrackCatalog.get_map_definition(map_id) == null, "%s should not remain in the runtime map catalog" % map_id)
	for scene_path in [
		"res://assets/gameplay/tracks/home_estate_v1/home_estate_v1_map.tscn",
		"res://assets/gameplay/tracks/home_plan50_622_v1/home_plan50_622_v1_map.tscn",
	]:
		assert_true(not ResourceLoader.exists(scene_path), "%s should be removed from runtime assets" % scene_path)
